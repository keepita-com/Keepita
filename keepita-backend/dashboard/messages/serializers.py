...
class MessageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Message
        fields = [
            'id', 'date', 'body', 'status', 'seen', 'sim_slot',
...
class ChatThreadListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ChatThread
        fields = [
            'id', 'address', 'contact', 'created_at',
            'last_message', 'last_message_date', 'messages_count', 'unread_count'
        ]
    
    def get_last_message(self, obj):
        last_message = obj.messages.order_by('-date').first()
        if last_message:
            return last_message.body[:100] + ('...' if len(last_message.body or '') > 100 else '')
        return None
...
class ChatThreadOverviewSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'address', 'contact', 'created_at',
            'last_message', 'last_message_date', 'messages_count', 
            'unread_count', 'first_message_date', 'date_range'
        ]
