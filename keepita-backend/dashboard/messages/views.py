...
from .serializers import (ChatThreadDetailSerializer, ChatThreadListSerializer,
                          ChatThreadOverviewSerializer, MessageSerializer)
...
class ChatThreadViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def summary(self, request, *args, **kwargs): 
        ...
        threads_with_messages = queryset.filter(messages__isnull=False).distinct().count()
        ...
        return Response({
            ...
            'totals': {
                ...
                'avg_messages_per_thread': round(message_stats['avg_messages_per_thread'] or 0, 2)
            },
            'most_active_threads': list(most_active)
        })

thread_pk_param = openapi.Parameter(
    'thread_pk', 
    openapi.IN_PATH, 
...
@swagger_auto_schema(manual_parameters=[backup_pk_param, thread_pk_param])  
class ThreadMessageViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Message.objects.filter(
            ...
            'chat_thread', 'chat_thread__contact'
        ).order_by('-date')
    ...
    def statistics(self, request, backup_pk=None, thread_pk=None):
        ...
        status_breakdown = messages.values('status').annotate(
            count=Count('id')
        ).order_by('status')
        
        
        sim_breakdown = messages.values('sim_slot').annotate(
            count=Count('id')
        ).order_by('sim_slot')
        ...
        return Response({
            ...
            'totals': {
                ...
                'read_percentage': round((total_messages - unread_messages) / total_messages * 100, 2) if total_messages > 0 else 0
            },
            'date_range': {
                'first_message': first_message.date if first_message else None,
                'last_message': last_message.date if last_message else None,
                'duration_days': (last_message.date - first_message.date).days if first_message and last_message else 0
            },
