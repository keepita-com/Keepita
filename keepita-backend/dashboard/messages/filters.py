...
class ChatThreadFilter(django_filters.FilterSet):
    ...
    contact_name = django_filters.CharFilter(
        field_name="contact__name", lookup_expr="icontains"
    )
    contact_phone = django_filters.CharFilter(
        field_name="contact__phone_number", lookup_expr="icontains"
    )
    is_favorite_contact = django_filters.BooleanFilter(
        field_name="contact__is_favorite"
    )
    ...
    def filter_min_messages(self, queryset, name, value):
        return queryset.annotate(messages_count=Count("messages")).filter(
            messages_count__gte=value
        )

    def filter_max_messages(self, queryset, name, value):
        return queryset.annotate(messages_count=Count("messages")).filter(
            messages_count__lte=value
        )
    ...
    def filter_last_message_after(self, queryset, name, value):
        return queryset.annotate(last_message_date=Max("messages__date")).filter(
            last_message_date__gte=value
        )

    def filter_last_message_before(self, queryset, name, value):
        return queryset.annotate(last_message_date=Max("messages__date")).filter(
            last_message_date__lte=value
        )
...
class MessageFilter(django_filters.FilterSet):
    ...
    contact_name = django_filters.CharFilter(
        field_name="chat_thread__contact__name", lookup_expr="icontains"
    )
    contact_phone = django_filters.CharFilter(
        field_name="chat_thread__contact__phone_number", lookup_expr="icontains"
    )
    address = django_filters.CharFilter(
        field_name="chat_thread__address", lookup_expr="icontains"
    )
