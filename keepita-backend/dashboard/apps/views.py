...
from .serializers import (
    ApkListDetailSerializer,
    ApkListSerializer,
    ApkOverviewSerializer,
    ApkPermissionSerializer,
)
...
class ApkListViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        queryset = ApkList.objects.select_related("backup").prefetch_related(
            "permissions"
        )
    ...
    def recently_used(self, request):
    ...
    def overview(self, request):
        ...
        total_permissions = ApkPermission.objects.filter(apk__in=queryset).count()

        most_recent_app = (
            queryset.filter(last_time_used__isnull=False)
            .order_by("-last_time_used")
            .first()
        )

        largest_app = queryset.filter(size__isnull=False).order_by("-size").first()

        apps_by_size_range = {
            "small": queryset.filter(size__lt=10 * 1024 * 1024).count(),
            "medium": queryset.filter(
                size__gte=10 * 1024 * 1024, size__lt=100 * 1024 * 1024
            ).count(),
        ...
        data = {
            ...
            "most_recent_app": (
                ApkListSerializer(most_recent_app, context={"request": request}).data
                if most_recent_app
                else None
            ),
            "largest_app": (
                ApkListSerializer(largest_app, context={"request": request}).data
                if largest_app
                else None
            ),
...
class AppPermissionsView(APIView):
    ...
    def get(self, request, backup_pk, app_id):
        ...
        permissions = ApkPermission.objects.filter(apk=app).select_related(
            "apk", "backup"
        )
        ...
        if permission_group:
            permissions = permissions.filter(
                permission_group__icontains=permission_group
            )
        ...
        permissions_serializer = ApkPermissionSerializer(
        ...
        response_data = {
            ...
            "stats": {
                ...
                "unique_groups": permissions.values("permission_group")
                .distinct()
                .count(),
                "protection_levels": list(
                    permissions.values_list("protection_level", flat=True).distinct()
                ),
            },
        }
