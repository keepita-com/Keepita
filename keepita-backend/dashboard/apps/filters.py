...
class ApkListFilter(django_filters.FilterSet):
    ...
    version_name = django_filters.CharFilter(
        field_name="version_name", lookup_expr="icontains"
    )
    ...
    last_used_after = django_filters.DateTimeFilter(
        field_name="last_time_used", lookup_expr="gte"
    )
    last_used_before = django_filters.DateTimeFilter(
        field_name="last_time_used", lookup_expr="lte"
    )
    ...
    def filter_permission_name(self, queryset, name, value):
        if value:
            return queryset.filter(
                permissions__permission_name__icontains=value
            ).distinct()
        return queryset
...
class ApkPermissionFilter(django_filters.FilterSet):
    ...
    apk_name = django_filters.CharFilter(
        field_name="apk__apk_name", lookup_expr="icontains"
    )
    permission_name = django_filters.CharFilter(
        field_name="permission_name", lookup_expr="icontains"
    )
    permission_group = django_filters.CharFilter(
        field_name="permission_group", lookup_expr="icontains"
    )
