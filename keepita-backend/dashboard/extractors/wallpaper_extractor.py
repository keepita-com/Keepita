...
class WallpaperExtractor(BaseExtractor):
    def extract(self) -> int:
        ...
        wallpaper_paths = [
            self._get_file_path(
                "WALLPAPER", "WALLPAPER_ext", "wallpaper", "wallpaper.png"
            ),
            self._get_file_path(
                "WALLPAPER", "WALLPAPER_ext", "wallpaper", "lock_wallpaper.png"
            ),
        ...
        self.update_progress(
            step_number, step_name, "Searching for wallpaper files", 20
        )
        ...
        with transaction.atomic():
            for idx, path in enumerate(wallpaper_paths):
                ...
                if path.exists() and path.is_file():
                    try:
                        wallpaper_type = (
                            "lock" if "lock" in path.name.lower() else "home"
                        )

                        existing_wallpaper = Wallpaper.objects.filter(
                            backup_id=self.backup_id, original_path=str(path)
                        ).first()
                        ...
                        with open(path, "rb") as image_file:
                            ...
                            wallpaper.image.save(
                                original_filename, django_file, save=False
                            )
        ...
        if wallpaper_count > 0:
            ...
            self.update_progress(
                step_number, step_name, "No wallpapers found", 100, "completed"
            )
