...
class WorldClockExtractor(BaseExtractor):
    ...
    def extract(self) -> int:
        ...
        worldclock_file = self._get_file_path('WORLDCLOCK', 'WORLDCLOCK_ext', 'worldclock_decrypted.xml')
        self.log_info(f"Extracting world clocks from: {worldclock_file}")
        
        clock_count = 0
        if not worldclock_file.exists():
            self.update_progress(step_number, step_name, 'World clock file not found', 0, 'failed')
            self.log_error(f"World clock file not found at: {worldclock_file}")
            return 0
            
        self.update_progress(step_number, step_name, 'Starting world clock extraction', 0)
        ...
        try:
            ...
            for i, clock_element in enumerate(clocks):
                try:
                    clock_id = int(clock_element.find('_id').text) if clock_element.find('_id') is not None else None
                    city = clock_element.find('city').text if clock_element.find('city') is not None else None
                    timezone = clock_element.find('gmt').text if clock_element.find('gmt') is not None else None
                    
                    if not city or not timezone:
                        continue
                    
                    dst_offset = int(clock_element.find('dst').text) if (clock_element.find('dst') is not None and 
                                                                        clock_element.find('dst').text != '-1') else 0
                    home_zone_id = int(clock_element.find('homezone').text) if clock_element.find('homezone') is not None else 0
                    point_x = int(clock_element.find('pointX').text) if (clock_element.find('pointX') is not None and 
                                                                        clock_element.find('pointX').text != '-1') else 0
                    point_y = int(clock_element.find('pointY').text) if (clock_element.find('pointY') is not None and 
                                                                        clock_element.find('pointY').text != '-1') else 0
