...
class IOSContactExtractor(BaseExtractor):

    def extract(self):
        self.log_info(f"[Backup {self.backup_id}] Starting iOS contact and call log import from server data.")
        ...
        with transaction.atomic():
            ...
            for i, contact_data in enumerate(items):
                try:
                    ...
                    phone_number = contact_data.get('phone_number', contact_data.get('phone', ''))
                    ...
                    if phones and not phone_number:
                        for phone in phones:
                            if isinstance(phone, dict):
                                phone_number = phone.get('number', phone.get('value', ''))
                            else:
                    ...
                    if i > 0 and i % 50 == 0 and total_contacts > 0:
                    ...
                    self.log_error(f"Error importing contact: {e}")
    ...
    def _import_call_logs_from_json(self, json_data: dict, step_number: int, step_name: str) -> int:
        ...
        with transaction.atomic():
            ...
            for i, call_data in enumerate(items):
                try:
                    number = call_data.get('number', call_data.get('address', ''))
                    name = call_data.get('name', '')
                    call_type = call_data.get('type', call_data.get('call_type', 'INCOMING'))
