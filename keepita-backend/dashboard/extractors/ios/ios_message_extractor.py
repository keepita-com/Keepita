...
class IOSMessageExtractor(BaseExtractor):

    def extract(self) -> int:
        step_number = 8
        step_name = 'messages'
        
        self.log_info(f"[Backup {self.backup_id}] Starting iOS message import from server data.")
        self.update_progress(step_number, step_name, 'Starting message import', 0)
    ...
    def _import_messages_from_json(self, json_data: dict, step_number: int, step_name: str) -> int:
        ...
        with transaction.atomic():
            ...
            for i, msg_data in enumerate(items):
                try:
                    address = msg_data.get('address', msg_data.get('handle_id', msg_data.get('sender', 'Unknown')))
                    ...
                    if date_str:
                        try:
                            if isinstance(date_str, (int, float)):
                                if date_str < 1e9:
                                    msg_date = datetime(2001, 1, 1) + timedelta(seconds=date_str)
                                else:
        ...
        self.log_info(f"Successfully imported {message_count} messages in {thread_count} threads.")
