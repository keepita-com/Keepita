...
class IOSNoteExtractor(BaseExtractor):

    def extract(self) -> int:
        step_number = 15
        step_name = 'notes'
        
        self.log_info(f"[Backup {self.backup_id}] Starting iOS notes import from server data.")
        self.update_progress(step_number, step_name, 'Starting notes import', 0)
        ...
        with transaction.atomic():
            ...
            for note_data in items:
                try:
                    ...
                    body = note_data.get('body', note_data.get('content', note_data.get('text', '')))
                    note_id = note_data.get('note_id', note_data.get('id'))
                    ...
                    self.log_error(f"Error importing note: {e}")

        self.log_info(f"Successfully imported {note_count} notes.")
