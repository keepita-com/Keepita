...
class WifiNetworkViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        if getattr(self, 'swagger_fake_view', False):
            return WifiNetwork.objects.none()
    ...
    def statistics(self, request, *args, **kwargs): 
        ...
        security_breakdown = queryset.values('security_type').annotate(
            count=Count('id')
        ...
        frequency_distribution = {
            ...
            'unknown': queryset.filter(Q(frequency__isnull=True) | Q(frequency='')).count(),
        }
    ...
    @action(detail=False, methods=['get'])
    def by_security(self, request, *args, **kwargs): 
        queryset = self.filter_queryset(self.get_queryset())
        
        security_groups = {}
        for security_type, _ in WifiNetwork.SECURITY_TYPES:
            networks = queryset.filter(security_type=security_type).order_by('ssid')
            security_groups[security_type] = WifiNetworkSerializer(networks, many=True).data
        
        return Response(security_groups)

    @action(detail=False, methods=['get'])
    def by_frequency(self, request, *args, **kwargs): 
        queryset = self.filter_queryset(self.get_queryset())
    ...
    @action(detail=False, methods=['get'])
    def saved_networks(self, request, *args, **kwargs): 
        queryset = self.filter_queryset(self.get_queryset())
    ...
    @action(detail=False, methods=['get'])
    def open_networks(self, request, *args, **kwargs): 
        queryset = self.filter_queryset(self.get_queryset())
    ...
    @action(detail=False, methods=['get'])
    def hidden_networks(self, request, *args, **kwargs): 
        queryset = self.filter_queryset(self.get_queryset())
    ...
    @action(detail=False, methods=['get'])
    def recent_connections(self, request, *args, **kwargs): 
        from datetime import timedelta
    ...
    @action(detail=False, methods=['get'])
    def security_analysis(self, request, *args, **kwargs): 
        queryset = self.filter_queryset(self.get_queryset())
        ...
        low_risk = queryset.filter(security_type__in=['WPA2_PSK', 'WPA_WPA2_PSK', 'SAE', 'EAP']).count()
        
        risk_assessment = {
            'high_risk': {'count': high_risk, 'percentage': round((high_risk / total) * 100, 2)},
            'medium_risk': {'count': medium_risk, 'percentage': round((medium_risk / total) * 100, 2)},
            'low_risk': {'count': low_risk, 'percentage': round((low_risk / total) * 100, 2)},
        }
        ...
        return Response({
            ...
            'recommendations': self._get_security_recommendations(high_risk, medium_risk, low_risk)
        })
