...
class WifiNetworkFilter(django_filters.FilterSet):
    ...
    is_saved = django_filters.BooleanFilter(
        field_name="is_saved", label="Saved Network"
    )
    has_password = django_filters.BooleanFilter(
        method="filter_has_password", label="Has Password"
    )
    ...
    last_connected_from = django_filters.DateTimeFilter(
        field_name="last_connected", lookup_expr="gte", label="Last Connected From"
    )
    last_connected_to = django_filters.DateTimeFilter(
        field_name="last_connected", lookup_expr="lte", label="Last Connected To"
    )
    ...
    backup_name = django_filters.CharFilter(
        field_name="backup__name", lookup_expr="icontains", label="Backup Name"
    )
