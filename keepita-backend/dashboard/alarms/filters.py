...
class AlarmFilter(django_filters.FilterSet):
    ...
    time_from = django_filters.TimeFilter(field_name='time', lookup_expr='gte', label='Time From')
    time_to = django_filters.TimeFilter(field_name='time', lookup_expr='lte', label='Time To')
    
    repeat_type = django_filters.NumberFilter(field_name='repeat_type', label='Repeat Type')
    ...
    class Meta:
        model = Alarm
        fields = ['active', 'repeat_type', 'time_from', 'time_to', 'created_from', 'created_to']
