...
class AlarmViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = Alarm.objects.filter(backup__user=self.request.user).select_related(
            "backup"
        )
    ...
    def statistics(self, request):
        ...
        common_times = (
            queryset.values("time").annotate(count=Count("id")).order_by("-count")[:5]
        )
    ...
    @action(detail=False, methods=["get"])
    def by_time_period(self, request):
        queryset = self.filter_queryset(self.get_queryset())
    ...
    @action(detail=False, methods=["get"])
    def recurring_alarms(self, request):
        queryset = self.filter_queryset(self.get_queryset())
    ...
    @action(detail=False, methods=["get"])
    def one_time_alarms(self, request):
        queryset = self.filter_queryset(self.get_queryset())
