...
from .serializers import (
    BluetoothDeviceDetailSerializer,
...
class BluetoothDeviceViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def paired(self, request):
        queryset = self.get_queryset().filter(bond_state__gt=0)
        serializer = self.get_serializer(queryset, many=True)
        return Response({serializer.data})

    @action(detail=False, methods=["get"])
    def recently_connected(self, request):
        queryset = (
            self.get_queryset()
            .filter(last_connected__isnull=False)
    ...
    def statistics(self, request):
        ...
        device_classes_breakdown = {
            str(item["device_class"]): item["count"] for item in device_classes
        }

        last_paired_device = (
            queryset.filter(bond_state__gt=0).order_by("-created_at").first()
        )
        most_recent_connection = (
            queryset.filter(last_connected__isnull=False)
            .order_by("-last_connected")
            .first()
        )
        ...
        return Response({"backup_id": backup_pk, "overview": serializer.data})
    ...
    def search_devices(self, request):
        ...
        for device in devices:
            ...
            if query.lower() in highlighted_address.lower():
                highlighted_address = highlighted_address.replace(
                    query, f"<mark>{query}</mark>"
                )
